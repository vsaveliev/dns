...
Activate: 20110302104537`
	...
	k := &DNSKEY{
		Hdr: RR_Header{"miek.nl.", TypeDNSKEY, ClassINET, 3600, 0},
		Flags: 256,
		Protocol: 3, // always for dnssec
	}
...
PrivateKey: WURgWHCcYIYUPWgeLmiPY2DJJk02vgrmTfitxgqcL4vwW7BOrbawVmVe0d9V94SR`
	...
	k := &DNSKEY{
		Hdr: RR_Header{"example.net.", TypeDNSKEY, ClassINET, 3600, 0},
		Flags: 257,
		Protocol: 3, // always for dnssec
	}

	privkey, err := k.NewPrivateKey(priv)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := sig.Verify(k, []RR{a}); err != nil {
		t.Fatalf("failure to validate:\n%s\n%s\n%s\n\n%s\n\n%v",
			k.String(),
			a.String(),
			sig.String(),
			k.PrivateKeyString(privkey),
			err,
		)
	}
}
